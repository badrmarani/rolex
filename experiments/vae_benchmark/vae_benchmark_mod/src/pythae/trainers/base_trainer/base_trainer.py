...
class BaseTrainer:
    ...
    def __init__(
        ...
        training_signature: str = None,
    ):

        self.repeat_number = repeat_number
        self.training_signature = training_signature
    ...
    def _set_output_dir(self):
        ...
        save_path = os.path.join(
            ...
            f"{self.model_name}_training_{self.training_signature}"
        )
        os.makedirs(save_path, exist_ok=True)

        self.training_dir = os.path.join(
            save_path,
            f"{self.model_name}_rep_{self.repeat_number}"
        )

        if not os.path.exists(self.training_dir) and self.is_main_process:
            os.makedirs(self.training_dir, exist_ok=True)
            logger.info(
                f"Created {self.training_dir}. \n"
